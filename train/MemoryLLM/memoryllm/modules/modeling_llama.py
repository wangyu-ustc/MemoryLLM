    ...
    def __init__(self, config: LlamaConfig, layer_idx: Optional[int] = None):
        ...
        if self.add_selector:
            if self.config.map_from_hidden_states:
                ...
                # if hasattr(self.config, "num_selector_layers") and self.config.num_selector_layers > 1:
                #     if self.config.num_selector_layers == 2:
                #         self.query_proj = nn.Sequential(
                #             nn.Linear(self.hidden_size, config.selector_hidden_dim * 2, bias=False),
                #             ACT2FN[self.config.hidden_act],
                #             nn.Linear(config.selector_hidden_dim * 2, config.selector_hidden_dim, bias=False)
                #         )
                ...
                #         raise NotImplementedError
                # else:
                #     self.query_proj = nn.Linear(self.hidden_size, config.selector_hidden_dim, bias=False)
                #     self.key_proj = nn.Linear(self.hidden_size, config.selector_hidden_dim, bias=False)
            else:
                raise NotImplementedError
                # self.query_proj = nn.Linear(self.num_heads * self.head_dim, config.selector_hidden_dim, bias=False)
                # self.key_proj = nn.Linear(self.num_key_value_heads * self.head_dim, config.selector_hidden_dim, bias=False)
                # if hasattr(self.config, "add_encoder_retriever") and self.config.add_encoder_retriever:
                #     raise NotImplementedError
        ...
        if self.add_selector and output_retriever_weights:
            ...
            if self.config.map_from_hidden_states:
                ...
                if random_retriever_length:
                    ...
                    # indices = torch.randperm(sentence_hidden_states.size(1))[:length]
                    # indices = torch.sort(indices).values
                    # sentence_hidden_states = sentence_hidden_states[:, indices]
